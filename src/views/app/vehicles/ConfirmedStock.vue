<template>
   <div>
      <datatable-heading :title="$t('menu.confirmed-stock')" :changePageSize="changePageSize"
            ...
            <b-card>
               <vuetable ref="vuetable" class="order-with-arrow colored" :api-url="apiBase"
                  ...
                  <template slot="stock_status" slot-scope="props">
                    <b-input-group v-if="$can('vehicle_confirmed.update')">
...
export default {
   ...
   methods: {
			fetchData() {
				let url = apiUrl + "/confirmedstock?per_page=1"
				axios
				.get(url)
				.then(r => r.data)
				.then(res => {
